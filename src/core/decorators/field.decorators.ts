...
export function MongoIdField(
  ...
  const decorators = [Type(() => String), IsMongoId({ each: options.each })];
...
export function MongoIdFieldOptional(
  options: Omit<ApiPropertyOptions, 'type' | 'required' | 'isArray'> & IFieldOptions = {},
): PropertyDecorator {
  return applyDecorators(IsOptional({ each: options.each }), MongoIdField({ required: false, ...options }));
}
